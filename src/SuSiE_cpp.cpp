...
//' @export
// [[Rcpp::export]]
Rcpp::List SuSiE_group(arma::vec y,arma::mat X1,arma::mat X2,double L, double sigma2=0,double delta=0.001,int maxIt=100) {
  ...
  if (sigma2 == 0) {
    if (L > 0) {
      ...
      while (conv == 0) {
        ...
        for (int l = 0; l < L; l++) {
          ...
          Rcpp::List ser = SER(r_l,X,XX,1/(mu_q_sigma2inv),sigma2_0,pi_0);
        ...
        double B_q_sigma2 = B_sigma + 0.5*ERSS;
        double A_q_sigma2 = 0.5*n + A_sigma;
      ...
      arma::vec Sigma_q_beta1_out = sum(Sigma_q_beta1,1);
      arma::vec Sigma_q_beta2_out = sum(Sigma_q_beta2,1);
      ...
      return Rcpp::List::create(
        ...
        Rcpp::Named("SigmaB1") = Sigma_q_beta1_out,
        Rcpp::Named("SigmaB2") = Sigma_q_beta2_out,
    ...
    if (L == 0) {
      ...
      double B_q_sigma2 = B_sigma + 0.5*sum(y%y);
      double A_q_sigma2 = 0.5*n + A_sigma;
      mu_q_sigma2 = B_q_sigma2/(A_q_sigma2-1);
